...
The capture implementation launches pinned Chromium with `--force-device-scale-factor=2`, preserves a 1440×900 CSS viewport, and requires genuine 2880×1800 screencast pixels. It never calls `Emulation.setVisibleSize`. Before user actions begin, a temporary four-band CSS calibration probe is captured through `Page.startScreencast`; decoded physical samples prove that CSS pixels were painted at 2× rather than merely placed inside a dimension-correct 2880×1800 container. The probe is removed before the real capture. Runtime diagnostics retain browser metrics, CDP layout metrics, launch arguments, JPEG dimensions, and proof observations. Day-2 frame-only bundles omit `timeline.json`; captures containing actions include it.
